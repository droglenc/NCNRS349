...
* `rsmpls=` is the number of simulations to run for each scenario (default is `rsmpls=5000`).
...
For example, the plot above shows that the sampling distribution for the population abundance estimates is right-skewed.  In addition, the population abundance estimate is biased because the mean population abundance estimate is greater than the known initial population abundance (the green line is greater than the red line).  The bias is an overestimate by 2.2%.  In addition, an approximate 95% confidence interval for the population abundance estimate is from 784 to 1379.
...
Each simulation scenario below should be based on a known population size of `N=1000` individuals (the default), an expected number of marks of 100, and an expected number in the second sample of 100.  Once the sliders have been set at the requested levels, the `Rerandomize` button should be pressed several times to ensure that any conclusions that you draw are not based on the results of a single set of random simulation runs.
