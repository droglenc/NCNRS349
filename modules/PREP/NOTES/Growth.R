...
rqrd <- c("FSA","FSAdata","captioner","dplyr","knitr","xtable")
library(FSA)
library(FSAdata)
# for upperTriangle
library(gdata)
...
vb2 <- vbFuns("original")
fit2 <- nls(tl~vb2(age,Linf,K,L0),data=crm,
            start=vbStarts(tl~age,data=crm,type="original"))
Linf <- coef(fit2)[1]
L0 <- coef(fit2)[3]
# Plot the data
plot(tl~jitter(age),data=crm,xlab="Age",ylab="Total Length (mm)",pch=19)
curve(vb2(x,Linf=Linf,L0=L0,K=coef(fit2)[2]),from=1,to=10,lwd=3,add=TRUE)
# Plot the data
plot(tl~jitter(age),data=crm,xlab="Age",ylab="Total Length (mm)",ylim=c(0,470),xlim=c(-2,12),pch=19)
curve(vb2(x,Linf=Linf,L0=L0,K=coef(fit2)[2]),from=-2,to=12,lwd=3,add=TRUE)
# Mark L0 on plot
...
sL0    <- c(NA,coef2[3],NA,NA,NA,coef6[2])
sLinf  <- c(coef1[1],coef2[1],NA,NA,NA,coef6[1])
sK     <- c(coef1[2],coef2[2],coef3[3],NA,coef5[2],NA)
...
curve(vb4(x,L1=coef(fit4)[1],L2=coef(fit4)[2],L3=coef(fit4)[3],t1=t1,t3=t3),
      from=0,to=10,lwd=2,col="green",add=TRUE)
          ...
          links=c(Script="Growth.R",RMarkdown="Growth.Rmd"))


# Script created at 2021-12-29 21:03:26
