...
Data from before and after the regulation change were stored in the `LenAge` sheet of the `GoeddeCobleData.xlsx` Excel 2007 file  ([available from here](GoeddeCoble.xlsx)).    The variables in this data frame are defined as follows,
...
1. Organize your plots for ease of use (e.g., put all four one page in a 2x2 grid)
...
```{r fig.width=3.75, fig.height=3.75, fig.show="hold", echo=-c(1:3)}
library(readxl)
d <- read_excel("GoeddeCobleData.xlsx",sheet=2) %>%
  mutate(period=factor(period),species=factor(species)) %>%
  as.data.frame()
par(mar=c(3,3,1.5,1),mgp=c(1.9,0.5,0),tcl=-0.2)
